...
class ActualTermsRegistration extends React.Component {
  ...
  componentWillMount() {
    ...
    if (UserId === null) {
      ...
      axios
        .post(ip + '/get_all_term_iterations')
        .then(response => {
  ...
  handleSubmit(event) {
    ...
    if (
      this.state.SelectedTermIteration === '' ||
      this.state.TermStartDate === '' ||
      this.state.TermEndDate === ''
    ) {
      alert('Kindly fill in every field on the form');
    } else {
      var TermStartDate =
      ...
      axios
        .post(
          ...
          querystring.stringify({
            TermIterationId: this.state.SelectedTermIteration.value,
